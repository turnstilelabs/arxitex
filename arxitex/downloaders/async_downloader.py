...
class AsyncSourceDownloader:
    ...
    async def async_download_and_read_latex(self, arxiv_id: str) -> Optional[str]:
        ...
        try:
            ...
            if not tex_files:
                logger.error(
                    f"No .tex files found in the extracted source for {arxiv_id}."
                )
                return None

            logger.info(
                f"Found {len(tex_files)} .tex files: {[f.name for f in tex_files]}"
            )
            ...
            logger.error(
                f"An error occurred during the download/extraction process for {arxiv_id}: {e}"
            )
            return None
    ...
    async def find_tex_files(self, source_path: Path) -> List[Path]:
        ...
        def _blocking_find_tex_files(path: Path) -> List[Path]:
            ...
            if not tex_files:
                ...
                logger.error(
                    f"No .tex files found. Available files: {[f.name for f in all_files]}"
                )
                return []
        ...
        for tex_file in sorted(tex_files):  # Sort for deterministic order
            try:
                async with aiofiles.open(
                    tex_file, "r", encoding="utf-8", errors="ignore"
                ) as f:
