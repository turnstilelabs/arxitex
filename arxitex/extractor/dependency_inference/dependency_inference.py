...
class GraphDependencyInference:
    ...
    def infer_dependency(
        ...
        """
        prompt = self.prompt_generator.make_dependency_prompt(
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to infer dependency between artifacts: {source_artifact['id']} and {target_artifact['id']}"
            )

    async def ainfer_dependency(
        self, source_artifact: dict, target_artifact: dict
    ) -> PairwiseDependencyCheck:
