...
class BaseGraphBuilder:
    ...
    def build_graph(
        self, latex_content: str, source_file: Optional[str] = None
    ) -> DocumentGraph:
        ...
        for node in nodes:
            graph.add_node(node)
        logger.info(
            f"Pass 1: Parsed {len(graph.nodes)} artifacts and linked detached proofs."
        )

        # --- Pass 2: Resolve references and create all links (edges) ---
        edges_to_add, external_nodes_to_add = self._resolve_and_create_graph_links(
            graph.nodes
        )
    ...
    def _link_proofs(
        ...
        for node in nodes:
            ...
            for proof in proofs:
                if proof["used"] or not proof["optional_arg"]:
                    continue

                # Pattern to find a ref/cref to this node's label.
                ref_pattern = re.compile(
                    r"\\(?:[cC]ref|[vV]ref|[Aa]utoref|ref)\s*\{"
                    + re.escape(node.label)
                    + r"\}"
                )
                ...
                if ref_pattern.search(proof["optional_arg"]):
                    ...
                    proof["used"] = True
                    logger.debug(
                        f"Linked proof to artifact {node.id} via explicit label '{node.label}' in proof argument."
                    )
                    break
            ...
            for proof in sorted(proofs, key=lambda p: p["start_char"]):
                ...
                if node_end_char < proof["start_char"] < next_node_start:
                    if not proof["optional_arg"]:
                        ...
                        proof["used"] = True
                        logger.debug(
                            f"Linked proof to artifact {node.id} via proximity."
                        )
                        break
        ...
        for node in nodes:
            if node.proof:
                node.references.extend(
                    self._extract_references_from_content(None, node.proof)
                )
    ...
    def _extract_references_from_content(
        self, content: str, proof_content: Optional[str]
    ) -> List[Reference]:
        """Extract all references from artifact and proof content."""
        references = []
        ref_pattern = re.compile(
            r"\\(?:[cC]ref|[vV]ref|[Aa]utoref|ref|eqref)\s*\{([^}]+)\}"
        )
