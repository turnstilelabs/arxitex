...
from arxitex.detector.stupid_lemmas.stupid_lemmas_detection_models import (
    StupidLemmaDetectionResult,
)
...
class StupidLemmaDetector:
    ...
    def detect_stupid_lemma(self, statement: str) -> StupidLemmaDetectionResult:
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to detect stupid lemma for statement: {statement}"
            )


def process_statements(
    df: pd.DataFrame, statement_column: str, max_workers: int
) -> pd.DataFrame:
...
def main():
    ...
    parser.add_argument(
        "--statement-column", type=str, help="Column name containing statements"
    )
    parser.add_argument(
        "--max-workers", type=int, default=20, help="Maximum number of workers"
    )
    ...
    if args.statement:
        ...
        detect_lemmas(
            args.input_file, args.statement_column, args.output_file, args.max_workers
        )
    else:
        parser.error(
            "Either provide --statement for single test or --input-file and --statement-column for batch processing"
        )
