...
class DiscoveryIndex(BaseSQLiteIndex):
    ...
    def add_papers(self, new_papers: List[Dict[str, Any]]) -> int:
        ...
        papers_to_insert = [
            (p['arxiv_id'], self._serialize(p)) 
            for p in new_papers if p.get('arxiv_id')
        ]
    ...
    def get_pending_papers(self) -> List[Dict[str, Any]]:
        ...
        with self._get_connection() as conn:
            cursor = conn.execute("SELECT metadata FROM discovered_papers ORDER BY arxiv_id")
    ...
    def remove_paper(self, arxiv_id: str):
        ...
        with self._get_connection() as conn:
            conn.execute("DELETE FROM discovered_papers WHERE arxiv_id = ?", (arxiv_id,))
            conn.commit()