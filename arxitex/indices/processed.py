...
class ProcessedIndex(BaseSQLiteIndex):
    ...
    def is_successfully_processed(self, arxiv_id: str) -> bool:
        ...
        with self._get_connection() as conn:
            cursor = conn.execute("SELECT status FROM processed_papers WHERE arxiv_id = ?", (arxiv_id,))
    ...
    def get_paper_status(self, arxiv_id: str) -> Optional[Dict]:
        ...
        with self._get_connection() as conn:
            cursor = conn.execute("SELECT * FROM processed_papers WHERE arxiv_id = ?", (arxiv_id,))
            row = cursor.fetchone()
            if not row:
                return None
